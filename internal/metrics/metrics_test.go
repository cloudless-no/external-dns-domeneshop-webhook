...
func Test_OpenMetrics_SetFilteredOutDomains(t *testing.T) {
	metrics = nil
	const val = 5
	expected := float64(val)

	GetOpenMetricsInstance().SetFilteredOutDomains(val)
	actual := testutil.ToFloat64(metrics.filteredOutDomains)
...
func Test_OpenMetrics_SetSkippedRecords(t *testing.T) {
	...
	GetOpenMetricsInstance().SetSkippedRecords(testDomain, val)
