...
func GetOpenMetricsInstance() *OpenMetrics {
	if metrics == nil {
		...
		metrics = &OpenMetrics{
			...
			successfulApiCallsTotal: prometheus.NewCounterVec(
				prometheus.CounterOpts{
					...
					Help: "The number of successful Domeneshop API calls",
				},
				[]string{"action"},
			),
			failedApiCallsTotal: prometheus.NewCounterVec(
				prometheus.CounterOpts{
					...
					Help: "The number of Domeneshop API calls that returned an error",
				},
				[]string{"action"},
			),
			filteredOutDomains: prometheus.NewGauge(prometheus.GaugeOpts{
				Name: "filtered_out_domains",
				Help: "The number of domains excluded by the domain filter",
			}),
			...
			apiDelayHist: prometheus.NewHistogramVec(
				prometheus.HistogramOpts{
					Name:    "api_delay_hist",
					Help:    "Histogram of the delay in milliseconds when calling the Domeneshop API",
		...
		reg.MustRegister(metrics.filteredOutDomains)
...
// SetFilteredOutDomains sets the value for the filtered_out_domains gauge.
func (m *OpenMetrics) SetFilteredOutDomains(num int) {
	m.filteredOutDomains.Set(float64(num))
}

// SetSkippedRecords sets the value for the skipped_records gauge.
func (m *OpenMetrics) SetSkippedRecords(domain string, num int) {
	label := prometheus.Labels{"domain": domain}
