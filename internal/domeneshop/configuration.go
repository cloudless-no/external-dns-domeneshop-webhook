...
// Configuration contains the Domeneshop provider's configuration.
type Configuration struct {
	// DNS token key for authentication
	Token string `env:"DOMENESHOP_TOKEN" required:"true"`
	// DNS Secret key for authentication
	Secret string `env:"DOMENESHOP_SECRET" required:"true"`
	// If true, do not execute actions on the API
	DryRun bool `env:"DRY_RUN" default:"false"`
	// Enable debugging logs
	Debug bool `env:"DOMENESHOP_DEBUG" default:"false"`
	...
	// Domains cache TTL in seconds.
	DomainCacheTTL int `env:"DOMAIN_CACHE_TTL" default:"0"`
	// Enable bulk mode
...
func GetDomainFilter(config Configuration) *endpoint.DomainFilter {
	...
	createMsg := "Creating Domeneshop provider with "
	...
	if config.RegexDomainFilter != "" {
		...
		if len(config.DomainFilter) > 0 {
			createMsg += fmt.Sprintf("domainNode filter: '%s', ", strings.Join(config.DomainFilter, ","))
		}
