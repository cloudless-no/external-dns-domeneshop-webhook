...
> [!IMPORTANT]
> Support for the legacy DNS system is going to be discontinued by Domeneshop in
...
### Legacy DNS API
A [DNS API token](https://docs.domeneshop.com/dns-console/dns/general/api-access-token/)
...
A [Cloud API token](https://docs.domeneshop.com/cloud/api/getting-started/generating-api-token/)
...
sidecars:
  - name: domeneshop-webhook
    image: ghcr.io/mconfalonieri/external-dns-domeneshop-webhook:{{ .Version }}
    ports:
      - containerPort: 8888
        name: webhook
      - containerPort: 8080
...
- the domain must be migrated to Domeneshop Cloud console
...
The Cloud API now supports a new way of updating the records for a domain called
**bulk mode**. This mode is activated by setting the `BULK_MODE` environment
variable to `true`. It works by exporting the domainfile, editing it and then
uploading the modified version. It is meant to be used in environments with a
high number of record changes per domain and a relatively long interval between
...
> and subject to "race conditions" if manual edits are applied while the domain
...
It comes with some limitations.
  ...
  1. [Domeneshop labels](#domeneshop-labels) are not supported, as there is no way to
     import them in the domainfile. Record comments are unsupported as well.
  ...
  4. If the domains managed by this webhook are also manipuilated by other
     software the following situation, although unlikely, could happen:
       
       1. the webhook downloads the domainfile for a domain
       2. the other software manipulates some record on the same domain
       3. the webhook uploads the domainfile, missing the changes applied by the
          other software
       4. since the upload rewrites the domainfile, those changes are now lost.
  
Please check the [Domain file import](https://docs.domeneshop.cloud/reference/cloud#tag/domains/domain-file-import)
section of the Domeneshop documentation for more details.
...
export and manipulate the domainfiles instead of the single recordsets. This will
reduce the API calls when updating domains with lots of changes and a relatively
long interval.
...
**DOMAIN_CACHE_TTL** that controls the TTL of the newly implemented domains
...
- [Domeneshop labels](#domeneshop-labels) are available when the new Cloud API is in use;
- The minimum ExternalDNS version is now **0.19.0** as the label system and the
  Cloud API are untested with previous versions.

Please notice that the Cloud API requires migrating the DNS domains to the new
DNS tab in Domeneshop console.
...
| --------------- | -------------------------------------- | -------------------------- |
| HETZNER_API_KEY | Domeneshop API token                      | Mandatory                  |
| BATCH_SIZE      | Number of domains per call               | Default: `100`, max: `100` |
...
> in **HETZNER_API_KEY** must be a Domeneshop Cloud token, NOT the classic DNS one.
...
to work with Domeneshop. It also contains huge parts from [DrBu7cher's Domeneshop provider](https://github.com/DrBu7cher/external-dns/tree/readding_hcloud_provider).
